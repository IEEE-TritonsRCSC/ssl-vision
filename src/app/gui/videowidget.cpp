...
VideoWidget::VideoWidget(QString title, QWidget * vis)
{
  ...
  actionCoordinateGraph = new QAction(this);
  actionCoordinateGraph->setObjectName("actionCoordinateGraph");
  actionCoordinateGraph->setCheckable(true);
  actionCoordinateGraph->setIcon(QIcon(":/icons/trapezoid.png"));
  actionCoordinateGraph->setShortcut(QKeySequence("Ctrl+g"));
  actionCoordinateGraph->setToolTip("Toggle Coordinate Graph (Ctrl-g)");
  actionCoordinateGraph->setShortcutContext(Qt::WidgetShortcut);
  if (vis!=0)
  ...
  if (vis!=0) {
    ...
    for (int i=0;i<toolactions.size()/2;i++) {
      std::swap(toolactions[i], toolactions[toolactions.size()-(i+1)]);
    }
...
void VideoWidget::toggleCoordinateGraph(bool val)
{
