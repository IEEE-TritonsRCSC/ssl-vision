    ...
    while(true) {
      if (rb!=0) {
        ...
        if ((capture != nullptr) && (capture->isCapturing())) {
          ...
          if (!bSuccess) {
            fprintf(stderr, "CaptureThread: copyAndConvertFrame failed - frame may be incomplete\n");
            fprintf(stderr, "  pic_raw width=%d height=%d data=%p\n",
                    pic_raw.getWidth(), pic_raw.getHeight(), (void*)pic_raw.getData());
            fprintf(stderr, "  d->video width=%d height=%d data=%p\n",
                    d->video.getWidth(), d->video.getHeight(), (void*)d->video.getData());
          }
