...
PluginVisualize::PluginVisualize(
    ...
    _image_mask(mask), calib_plugin(nullptr){
...
ProcessResult PluginVisualize::process(
  ...
  if (_v_enabled->getBool()) {
    ...
    // Interactive field line selection
    if (_v_interactive_calibration->getBool()) {
      DrawInteractiveSelection(data, vis_frame);
    }
    ...
    if(_v_coordinate_graph->getBool()) {
      DrawCoordinateGraph(data, vis_frame);
    }
...
void PluginVisualize::setCalibrationPlugin(const PluginCameraCalibration* calib_plugin) {
  this->calib_plugin = calib_plugin;
}
...
void PluginVisualize::DrawInteractiveSelection(FrameData* data, VisualizationFrame* vis_frame) {
  (void)data;
  if (calib_plugin == nullptr) {
    return;
  }

  // Read selection state directly from the calibration plugin
  bool isInteractiveSelecting = calib_plugin->getIsSelectingFieldLine();
  if (!isInteractiveSelecting) {
    return;
  }

  QPointF startPoint = calib_plugin->getSelectionStartPoint();
  QPointF endPoint = calib_plugin->getSelectionEndPoint();

  // Determine color and label based on calibration mode
  rgb drawColor;
  std::string label;

  PluginCameraCalibration::FieldLineType lineType = calib_plugin->getCurrentLineType();
  switch (lineType) {
    case PluginCameraCalibration::LINE_TOP:
      drawColor.set(255, 255, 0); // Yellow
      label = "TOP";
      break;
    case PluginCameraCalibration::LINE_BOTTOM:
      drawColor.set(0, 255, 255); // Cyan
      label = "BOTTOM";
      break;
    case PluginCameraCalibration::LINE_LEFT:
      drawColor.set(255, 0, 255); // Magenta
      label = "LEFT";
      break;
    case PluginCameraCalibration::LINE_RIGHT:
      drawColor.set(0, 255, 0); // Green
      label = "RIGHT";
      break;
    case PluginCameraCalibration::LINE_TOP_GOAL:
      drawColor.set(255, 0, 0); // Red
      label = "TOP GOAL";
      break;
    case PluginCameraCalibration::LINE_BOTTOM_GOAL:
      drawColor.set(255, 165, 0); // Orange
      label = "BOTTOM GOAL";
      break;
    default:
      return;
  ...
  const int endpointRadius = 6;

  // Draw the selection line with thicker style
  ...
  vis_frame->data.drawFatBox(
      startPoint.x() - endpointRadius,
      startPoint.y() - endpointRadius,
      endpointRadius * 2,
      endpointRadius * 2,
      drawColor);
  vis_frame->data.drawFatBox(
  ...
  // Draw line type label near the selection
  int labelX = (startPoint.x() + endPoint.x()) / 2;
  int labelY = (startPoint.y() + endPoint.y()) / 2 - 15;
  vis_frame->data.drawString(labelX - 40, labelY, label, drawColor);
}

void PluginVisualize::DrawCustomCalibrationSelection(FrameData* data, VisualizationFrame* vis_frame) {
  (void)data;
  if (calib_plugin == nullptr) {
    return;
  }
  if (!calib_plugin->getIsCustomCalibrationMode()) {
    return;
  }

  int count = calib_plugin->getCustomCalibrationPointCount();
  if (count <= 0) {
    return;
  }

  const QPointF* points = calib_plugin->getCustomCalibrationPoints();
  rgb drawColor;
  drawColor.set(255, 165, 0); // Orange
  const int endpointRadius = 6;
  ...
  for (int i = 0; i < count; ++i) {
    ...
    vis_frame->data.drawFatBox(
        points[i].x() - endpointRadius,
        points[i].y() - endpointRadius,
        endpointRadius * 2,
        endpointRadius * 2,
        drawColor);
  }
...
void PluginVisualize::DrawCoordinateGraph(FrameData* data, VisualizationFrame* vis_frame) {
  const int width = vis_frame->data.getWidth();
  const int height = vis_frame->data.getHeight();

  // Semi-transparent colors for the overlay
  const rgb major_grid_color(255, 255, 255);
  const rgb minor_grid_color(200, 200, 200);
  const rgb axis_color(255, 255, 0);
  const rgb label_color(255, 255, 255);
  
  const double major_grid_spacing = 1000.0; // 1000mm = 1m
  const double minor_grid_spacing = 500.0;  // 500mm

  // Calculate field bounds for grid generation
  double field_length = real_field.field_length->getDouble();
  double field_width = real_field.field_width->getDouble();
  
  // Draw grid lines from field origin (0,0) to field dimensions
  double x_start = 0.0;
  double x_end = field_length;
  double y_start = 0.0;
  double y_end = field_width;
  
  // Draw vertical grid lines
  for (double x = x_start; x <= x_end; x += minor_grid_spacing) {
    bool is_major = (fmod(x, major_grid_spacing) < 1.0);
    rgb grid_color = is_major ? major_grid_color : minor_grid_color;
    int thickness = is_major ? 2 : 1;
    
    GVector::vector3d<double> p1(x, y_start, 0.0);
    GVector::vector3d<double> p2(x, y_end, 0.0);
    
    // Transform field coordinates to image coordinates
    GVector::vector2d<double> img_p1, img_p2;
    camera_parameters.field2image(p1, img_p1);
    camera_parameters.field2image(p2, img_p2);
    
    // Check if points are within image bounds
    if (img_p1.x >= -100 && img_p1.x <= width + 100 && img_p1.y >= -100 && img_p1.y <= height + 100) {
      for (int t = 0; t < thickness; ++t) {
        vis_frame->data.drawLine(img_p1.x + t, img_p1.y, img_p2.x + t, img_p2.y, grid_color);
      }
    }
    ...
    if (is_major) {
      ...
      GVector::vector3d<double> label_pos(x, y_start - 100, 0.0);
      GVector::vector2d<double> img_label_pos;
      camera_parameters.field2image(label_pos, img_label_pos);
      if (img_label_pos.x > 0 && img_label_pos.y > 0 && img_label_pos.x < width && img_label_pos.y < height) {
        vis_frame->data.drawString(img_label_pos.x - 20, img_label_pos.y, label, label_color);
      }
    }
  }
  
  // Draw horizontal grid lines
  for (double y = y_start; y <= y_end; y += minor_grid_spacing) {
    bool is_major = (fmod(y, major_grid_spacing) < 1.0);
    rgb grid_color = is_major ? major_grid_color : minor_grid_color;
    int thickness = is_major ? 2 : 1;
    
    GVector::vector3d<double> p1(x_start, y, 0.0);
    GVector::vector3d<double> p2(x_end, y, 0.0);
    ...
    if (img_p1.x >= -100 && img_p1.x <= width + 100 && img_p1.y >= -100 && img_p1.y <= height + 100) {
      for (int t = 0; t < thickness; ++t) {
        vis_frame->data.drawLine(img_p1.x, img_p1.y + t, img_p2.x, img_p2.y + t, grid_color);
      }
    }
    ...
    if (is_major) {
      ...
      GVector::vector3d<double> label_pos(x_start - 100, y, 0.0);
      GVector::vector2d<double> img_label_pos;
      camera_parameters.field2image(label_pos, img_label_pos);
      if (img_label_pos.x > 0 && img_label_pos.y > 0 && img_label_pos.x < width && img_label_pos.y < height) {
