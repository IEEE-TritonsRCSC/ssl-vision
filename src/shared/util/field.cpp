...
RoboCupField::RoboCupField() {
  ...
  center_line = new VarBool(
    "Center Line", true);
  half_field = new VarBool(
    "Half Field", false);
  half_field_goal_side = new VarStringEnum(
    "Half Field Goal Side", "Right");
  half_field_goal_side->addItem("Left");
  half_field_goal_side->addItem("Right");
  field_rotation = new VarDouble(
      "Field Rotation", 0.0);
  ...
  settings->addChild(half_field);
  settings->addChild(half_field_goal_side);
  settings->addChild(field_rotation);
  ...
  connect(field_rotation,SIGNAL(wasEdited(VarType*)),this,SLOT(updateFieldLinesAndArcs()));
  connect(half_field,SIGNAL(wasEdited(VarType*)),this,SLOT(updateFieldLinesAndArcs()));
  connect(half_field_goal_side,SIGNAL(wasEdited(VarType*)),this,SLOT(updateFieldLinesAndArcs()));
...
RoboCupField::~RoboCupField() {
  ...
  delete center_line;
  delete half_field;
  delete half_field_goal_side;
  delete field_rotation;
...
void RoboCupField::rebuildFieldLinesAndArcs() {
  updateFieldLinesAndArcs();
}
...
void RoboCupField::updateFieldLinesAndArcs() {
    ...
    // Get rotation angle and precompute rotation matrix
    double rotation = field_rotation->getDouble();
    double cos_theta = cos(rotation);
    double sin_theta = sin(rotation);

    // Lambda to rotate a point
    auto rotatePoint = [&](double x, double y) -> std::pair<double, double> {
    ...
    bool half_field_enabled = half_field->getBool();
    bool goal_on_right = true;
    if (half_field_enabled) {
      goal_on_right = (half_field_goal_side->getString() == "Right");
    }
    ...
    std::pair<double, double> p1 = rotatePoint(x_min, fieldWidthHalf);
    std::pair<double, double> p2 = rotatePoint(x_max, fieldWidthHalf);
    field_lines.push_back(new FieldLine("TopTouchLine", p1.first, p1.second, p2.first, p2.second, line_thickness->getDouble()));

    p1 = rotatePoint(x_min, -fieldWidthHalf);
    p2 = rotatePoint(x_max, -fieldWidthHalf);
    field_lines.push_back(new FieldLine("BottomTouchLine", p1.first, p1.second, p2.first, p2.second, line_thickness->getDouble()));

    if (!half_field_enabled || !goal_on_right) {
      p1 = rotatePoint(-fieldLengthHalf, -fieldWidthHalf);
      p2 = rotatePoint(-fieldLengthHalf, fieldWidthHalf);
      field_lines.push_back(new FieldLine("LeftGoalLine", p1.first, p1.second, p2.first, p2.second, line_thickness->getDouble()));
    }

    if (!half_field_enabled || goal_on_right) {
      p1 = rotatePoint(fieldLengthHalf, -fieldWidthHalf);
      p2 = rotatePoint(fieldLengthHalf, fieldWidthHalf);
      field_lines.push_back(new FieldLine("RightGoalLine", p1.first, p1.second, p2.first, p2.second, line_thickness->getDouble()));
    }

    double halfway_x = half_field_enabled ? x_mid : 0.0;
    p1 = rotatePoint(halfway_x, -fieldWidthHalf);
    p2 = rotatePoint(halfway_x, fieldWidthHalf);
    field_lines.push_back(new FieldLine("HalfwayLine", p1.first, p1.second, p2.first, p2.second, line_thickness->getDouble()));

    if (!half_field_enabled || !goal_on_right) {
      p1 = rotatePoint(-penAreaX, -penAreaY);
      p2 = rotatePoint(-penAreaX, penAreaY);
      field_lines.push_back(new FieldLine("LeftPenaltyStretch", p1.first, p1.second, p2.first, p2.second, line_thickness->getDouble()));
    }

    if (!half_field_enabled || goal_on_right) {
      p1 = rotatePoint(penAreaX, -penAreaY);
      p2 = rotatePoint(penAreaX, penAreaY);
      field_lines.push_back(new FieldLine("RightPenaltyStretch", p1.first, p1.second, p2.first, p2.second, line_thickness->getDouble()));
    }

    if (!half_field_enabled || !goal_on_right) {
      p1 = rotatePoint(-fieldLengthHalf, -penAreaY);
      p2 = rotatePoint(-penAreaX, -penAreaY);
      field_lines.push_back(new FieldLine("LeftFieldLeftPenaltyStretch", p1.first, p1.second, p2.first, p2.second, line_thickness->getDouble()));

      p1 = rotatePoint(-fieldLengthHalf, penAreaY);
      p2 = rotatePoint(-penAreaX, penAreaY);
      field_lines.push_back(new FieldLine("LeftFieldRightPenaltyStretch", p1.first, p1.second, p2.first, p2.second, line_thickness->getDouble()));
    }

    if (!half_field_enabled || goal_on_right) {
      p1 = rotatePoint(fieldLengthHalf, -penAreaY);
      p2 = rotatePoint(penAreaX, -penAreaY);
      field_lines.push_back(new FieldLine("RightFieldRightPenaltyStretch", p1.first, p1.second, p2.first, p2.second, line_thickness->getDouble()));

      p1 = rotatePoint(fieldLengthHalf, penAreaY);
      p2 = rotatePoint(penAreaX, penAreaY);
      field_lines.push_back(new FieldLine("RightFieldLeftPenaltyStretch", p1.first, p1.second, p2.first, p2.second, line_thickness->getDouble()));
    }

    if (center_line->getBool()) {
      p1 = rotatePoint(x_min, 0);
      p2 = rotatePoint(x_max, 0);
      field_lines.push_back(new FieldLine("CenterLine", p1.first, p1.second, p2.first, p2.second, line_thickness->getDouble()));
    }
    ...
    // Load default arcs.
    // Center circle stays on the midline (full field) or the half-field edge.
    double center_x = half_field_enabled ? x_mid : 0.0;
    std::pair<double, double> center = rotatePoint(center_x, 0);
    // Adjust angles for rotation: add rotation angle to start and end angles
    double a1 = 0.0;
    double a2 = 2.0 * M_PI;
    if (half_field_enabled) {
      if (goal_on_right) {
    ...
    double a1_rot = a1 + rotation;
    double a2_rot = a2 + rotation;
    field_arcs.push_back(new FieldCircularArc("CenterCircle", center.first, center.second, center_circle_radius->getDouble(), a1_rot, a2_rot, 10));
